...
public class UserService extends ServiceBase {
    ...
    public List<UserView> getPerPage(int page) {
        List<User> Users = em.createNamedQuery(JpaConst.Q_EMP_GET_ALL, User.class)
    ...
    public long countByCode(String code) {
        ...
        long Users_count = (long) em.createNamedQuery(JpaConst.Q_EMP_COUNT_REGISTERED_BY_CODE, Long.class)
                .setParameter(JpaConst.JPQL_PARM_CODE, code)
                .getSingleResult();
        return Users_count;
    }
    ...
    /**
     * 従業員データを更新する
     * @param savedEmp 画面から入力された従業員の登録内容
     */
    private void update(UserView savedEmp) {

        em.getTransaction().begin();
        User e = findOneInternal(savedEmp.getId());
        UserConverter.copyViewToModel(e, savedEmp);
        em.getTransaction().commit();
